---
title: "DML-IVQR: L1-QR penalty based on Belloni and Chernozhukov (2011, Annals of Statistics) or cross-validation (quantreg)"
---
...
- **Function Output**
    ...
    - result[2] ====> GMM with each grid  
```{r}
DML_IVQR<-function(y,D,X,Z,tau,grid_alpha,core,CV=FALSE,CV_fold=5,penalty=seq(0,20,length=11)){                        #l1_norm TRUE=hqreg False=quantreg
  cl <- makeCluster(core)
  registerDoSNOW(cl)
  A=foreach(i = grid_alpha, .combine = "rbind") %dopar%{
    ...
    if(CV == FALSE){
      ...
      lasso=rq(y-i*D ~ X,tau=tau, method="lasso",lambda = cv_qr_penalty(y-i*D,X,tau=tau,penalty,CV_fold))
    }
...
qr50=ggplot(tgg, aes(x=GRID, y=GMM, colour=Model,group=Model)) + geom_line(size=1) +xlim(-1,3) +  geom_hline(aes(yintercept =  5.9915))
qr50
